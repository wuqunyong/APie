...
void DBProxyMgr::init()
{
	...
	auto rpcCB = [](const ::rpc_msg::CLIENT_IDENTIFIER& client, const std::string& args) -> std::tuple<uint32_t, std::string> {
		...
		{
			return std::make_tuple(::rpc_msg::CODE_Unregister, response.SerializeAsString());
		}
	...
	APie::RPC::RpcServerSingleton::get().registerOpcodes(rpc_msg::RPC_MysqlDescTable, rpcCB);
}
